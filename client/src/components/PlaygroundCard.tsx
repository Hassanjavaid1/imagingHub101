...
import { Code2, Copy, ArrowRight, Check } from "lucide-react";
...
export function PlaygroundCard({
  initialImageUrl = "https://imgapi.vercel.app/images/abc123.jpg",
  apiDocsHref = "#",
}: PlaygroundCardProps) {
    ...
    <Card className="border-slate-800 bg-slate-900/60">
      ...
      <CardContent className="space-y-5">
        ...
        <div className="space-y-1.5">
          <Label htmlFor="image-url" className="text-xs font-medium text-slate-300">
            Image URL
          </Label>
          <div className="flex gap-2">
            <Input
              ...
              placeholder="https://imgapi.vercel.app/images/abc123.jpg"
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <Input
                ...
                type="number"
                value={width}
                onChange={(event) => setWidth(parseNumberInput(event.target.value))}
          ...
          <div className="space-y-1.5">
            <Label htmlFor="height" className="text-xs font-medium text-slate-300">
              Height (h)
            </Label>
            <div className="relative">
              <Input
                ...
                type="number"
                value={height}
                onChange={(event) => setHeight(parseNumberInput(event.target.value))}
                className="border-slate-700 bg-slate-950/60 pr-9 text-sm text-slate-200"
              />
              <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-xs text-slate-500">
                px
          ...
          <div className="space-y-1.5">
            <Label htmlFor="quality" className="text-xs font-medium text-slate-300">
              Quality (q)
            </Label>
            <div className="relative">
              <Input
                ...
                max={100}
                value={quality}
                onChange={(event) => setQuality(parseNumberInput(event.target.value))}
          ...
          <div className="space-y-1.5">
            <Label htmlFor="format" className="text-xs font-medium text-slate-300">
              Format (f)
            </Label>
            <Select value={format} onValueChange={(value:string) => setFormat(value as ImageFormat)}>
              <SelectTrigger
                id="format"
        ...
        {/* Generated URL */}
        <div className="space-y-1.5">
          <Label htmlFor="generated-url" className="text-xs font-medium text-slate-300">
            Generated URL
          </Label>
          <div className="flex gap-2">
            <Input
        ...
        <div className="flex items-center justify-between pt-1">
          <p className="text-xs text-slate-500">
            Add parameters to the URL to resize, compress or convert your image on the fly.
          </p>
          <a
            href={apiDocsHref}
            className="flex shrink-0 items-center gap-1 text-xs font-medium text-cyan-400 hover:text-cyan-300"
          >
            View API Docs
